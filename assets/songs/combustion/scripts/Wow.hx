...
for(strum in playerStrums){
	FlxTween.tween(strum, {x: event == "in"? strum.x - 300: strum.x + 280}, 0.7, {ease:FlxEase.quadOut});
}
for(strum in cpuStrums){

	FlxTween.tween(strum, {alpha: event == "in" ? 0: 1}, 0.7, {ease:FlxEase.quadOut});

	FlxTween.tween(strum, {x: event == "in"? strum.x + 340 : strum.x - 300}, 0.7, {ease:FlxEase.quadOut});
}
...
function notesAlpha(event){
	FlxTween.tween(camNotes,{alpha:event =="in"?1:0},1);
}